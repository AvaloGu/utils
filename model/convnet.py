...
class ConvNeXt(nn.Module):
    ...
    def __init__(self, in_chans=3, num_classes=4, 
                 depths=[1, 1, 3, 1], dims=[48, 96, 192, 384], drop_path_rate=0.05, 
        ...
        # 18 drop path rates, one for each block. Stochastic depth decay rule
        dp_rates=[x.item() for x in torch.linspace(0, drop_path_rate, sum(depths))] # [0, 0.0118..., ..., 0.2]
        cur = 0
        for i in range(4):
            stage = nn.Sequential(
                # unpack the list of Block modules into individual arguments for nn.Sequential
        ...
        self.head = nn.Linear(dims[-1], num_classes) # (768, 4)
