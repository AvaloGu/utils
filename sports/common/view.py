...
class ViewTransformer:
    # transform homography perspectives
    def __init__(
            self,
        ...
        source = source.astype(np.float32) # cast to float32
        target = target.astype(np.float32)
        # this method finds homography matrix, it allows us to pass in more than 4 points
        self.m, _ = cv2.findHomography(source, target) # self.m is the homography matrix
        if self.m is None:
            raise ValueError("Homography matrix could not be calculated.")

    def transform_points(
            # this function uses homography matrix to move the points from one plane to another
            self,
        ...
        # homography matrix is 3x3 matrix, we need to expand the dimensionality of our points
        # because our points only exist on a 2D plane
        reshaped_points = points.reshape(-1, 1, 2).astype(np.float32)
        # this method bascially just applies homography matrix to transform the points
        transformed_points = cv2.perspectiveTransform(reshaped_points, self.m)
        return transformed_points.reshape(-1, 2).astype(np.float32) # remove that extra dim
